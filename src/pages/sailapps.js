...
function SailApps() {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      setIsConfirmed(true);
      setSendResult(
        'Your application can now access your data, you can close this window',
      );
    } catch (error) {
    ...
    <Layout noFooter title="SailPoint Application Authentication">
      <div className={styles.gettingStartedText}>
        ...
        <p className={styles.gettingStartedTwo}>
          Please confirm the application you are trying to access provides the
          same code as the one below
        </p>
        {state && (
          <div className={styles.gettingStartedThree}>
            <p>
              ...
              <span className={styles.bold}>
                {uuid.slice(uuid.length - 8, uuid.length - 4).toUpperCase()}-
                {uuid.slice(uuid.length - 4, uuid.length).toUpperCase()}
              </span>
            </p>
            {!isConfirmed && (
        ...
        {sendResult && (
          <p className={styles.gettingStartedThree}>{sendResult}</p>
        )}
      </div>
    </Layout>
  );
