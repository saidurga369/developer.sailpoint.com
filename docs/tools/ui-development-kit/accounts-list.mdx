...
export const load = async ({url, locals}) => {
  ...
  // highlight-next-line
  const reportResp = api.listAccounts({
    count: true,
    limit: Number(limit),
...
On the server side, the kit uses the `getFilters` and `getSorters` functions to get the filters and sorters from the URL. The kit then passes these functions to the [List Accounts endpoint](https://developer.sailpoint.com/docs/api/v3/list-accounts) to filter and sort the accounts.
